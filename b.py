...
cell = sheet.cell(row=1, column=5,value = "LastNumber_fail")
cell = sheet.cell(row=1, column=6,value = "FirstNumber_fail")
    ...
    for j in range(len(camera02)):
        if i == j:
            if camera01[i] == camera02[j]:
                ...
                end_row += 1
                sheet.cell(row = end_row +1 , column=5,value =camera01[i]) 
                sheet.cell(row = end_row +1 , column=6,value =camera02[j])
        else:
            continue
